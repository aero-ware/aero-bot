...
export default {
    ...
    async callback ({ message }): Promise<any> {
        if (games.has(message.channel.id)) return message.channel.send(
            new MessageEmbed()
                .setTitle("Error")
        ...
        const random = Math.floor(Math.random() * (CONFIG.MAX - CONFIG.MIN)) + CONFIG.MIN;

        message.channel.send(
            new MessageEmbed()
                .setTitle("Guess the number")
                .setDescription(`The number is between ${CONFIG.MIN} and ${CONFIG.MAX}\n You have **${CONFIG.TRIES}** tries to guess it.`)
                .setColor("RANDOM")
        );
...
async function play(message: Message, num: number): Promise<any> {
    ...
    if (collected.size === 0) return message.channel.send("Ending game, over 30 seconds of inactivity.");

    let argNum: number;
    try {
        argNum = parseInt(collected.first()!.content);
    } catch (e) { return play(message, num); }

    if (!argNum || isNaN(argNum)) return play(message, num);

    games.set(message.channel.id, games.get(message.channel.id)! + 1);

    if (games.get(message.channel.id)! > CONFIG.TRIES) return play(message, num);

    if (argNum === num) return message.channel.send(
        new MessageEmbed()
            .setTitle("The number has been found!")
            .setDescription(`You found the number in **${games.get(message.channel.id)}** tries!\n It was **${num}**`)
            .setColor("RANDOM")
            .setTimestamp()
    );
    ...
    message.channel.send(
        new MessageEmbed()
            .setTitle(`${argNum} is ${argNum > num ? "greater than" : "less than"} the secret number.`)
            .setDescription(`You have ${CONFIG.TRIES - games.get(message.channel.id)!} tries left.`)
            .setColor("RANDOM")
    );
