...
export default {
    ...
    category: "Moderation",
    description: "Deletes the specified number of messages, from a user if specified.",
    details: "Ping a user for the second argument to delete messages only from that user",
    args: true,
    usage: "<number> [user]",
    minArgs: 1,
    maxArgs: 2,
    ...
    async callback({ message, args }) {
        ...
        if (message.channel instanceof TextChannel) {
            if (!target) {
                ...
                return message.channel.bulkDelete(message.channel.messages.cache.filter(m => m.author.id === target.id));
            }
