    ...
    async callback({ message, args, client }): Promise<any> {
        ...
        switch (arg) {
            case "add":
                ...
                message.channel.send(`New rolemenu created in message ${messageID}`);
                return;
            ...
            case "update":
                ...
                if (!guildInfo.roleMenus || !guildInfo.roleMenus.has(messageID)) return message.reply(`message ${messageID} does not have a rolemenu attached to it. Use \`rolemenu add <message ID>\` to create one.`)
                    ...
                    new MessageEmbed()
                        ...
                        .addFields(
                            ...
                            },
                            {
                                ...
                                value: `Run \`rolemenu update <message ID> <emoji> <role ID>\` **in the same channel as the message**.\n` +
                                    '*The emoji will be automatically added as a reaction to the message.*',
                            },
                            {
                                name: 'Removing a role from a rolemenu',
                                value: `Run \`rolemenu update <message ID> <emoji> none\` **in the same channel as the message**.\n` + 
