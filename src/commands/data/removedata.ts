    ...
    async callback({ message, client }) {
        const confirmation = await message.channel.send(
            new MessageEmbed()
                ...
                .setDescription(
                    "Are you sure you want to remove ALL your data from the database?\n" +
        ...
        if (client.clientOptions.staff?.includes(message.author.id)) {
            message.channel.send(
                "You are a staff member. If you really want your data removed please contact the owner."
            );
            return;
        }
